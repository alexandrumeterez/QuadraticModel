#!/bin/bash
#SBATCH --job-name=spectrum_basis
...
LOG_SUBDIR="logs/$(date +%Y_%m_%d)_${JOB_ID}_basis"
mkdir -p "$LOG_SUBDIR"
cp -n "${BASH_SOURCE[0]}" "$LOG_SUBDIR/run_basis.sh"
...
"$PYTHON_BIN" -u analysis/spectrum/run_basis.py "${PY_ARGS[@]}"
